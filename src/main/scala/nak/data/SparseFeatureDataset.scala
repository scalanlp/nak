...
import nak.serialization.TextSerialization.Readable
import nak.serialization.{SerializationFormat, TextSerialization}
...
object SparseFeatureDataset {
  def fromSource[Output:TextSerialization.Readable](s: Source, name: String=""):SparseFeatureDataset[Output] = {
    ...
    val processed = for( ((output,indices,values),id) <- data.zipWithIndex) yield {
      val sparseVector = new SparseVector(indices.toArray,values.toArray,indices.length,maxIndex + 1)
