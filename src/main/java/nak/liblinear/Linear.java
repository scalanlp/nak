...
public class Linear {
	...
	public static Model loadModel(Reader inputReader) throws IOException {
		...
		while ((line = reader.readLine()) != null) {
			...
			if (split[0].equals("solver_type")) {
				...
				throw new RuntimeException("unknown text in model file: ["
						+ line + "]");
			}
		}
	...
	public static double predictValues(Model model, Feature[] x,
		...
		for (int i = 0; i < Math.min(dec_values.length, nr_w); i++)
			dec_values[i] = 0;
	 ...
	 * {@link java.util.Locale#ENGLISH} for number formatting.
	 */
	public static void saveModel(File modelFile, Model model)
			throws IOException {
	 ...
	 * </pre>
	 */
	private static void solve_l2r_l1l2_svc(Problem prob, double[] w,
			double eps, double Cp, double Cn, SolverType solver_type) {
		int l = prob.l;
		int w_size = prob.n;
		int i, s, iter = 0;
		double C, d, G;
	...
	private static void solve_l2r_l1l2_svr(Problem prob, double[] w,
		...
		while (iter < max_iter) {
			...
			for (s = 0; s < active_size; s++) {
				...
				beta[i] = Math.min(
						Math.max(beta[i] + d, -upper_bound[GETI_SVR(i)]),
		...
		v = 0.5 * v;
		for (i = 0; i < l; i++) {
			v += p * Math.abs(beta[i]) - y[i] * beta[i] + 0.5
					* lambda[GETI_SVR(i)] * beta[i] * beta[i];
			if (beta[i] != 0)
				nSV++;
		}
	...
	private static void solve_l2r_lr_dual(Problem prob, double w[], double eps,
		...
		while (iter < max_iter) {
			...
			for (s = 0; s < l; s++) {
				...
				if (C - z < 0.5 * C)
					z = 0.1 * z;
				double gp = a * (z - alpha_old) + sign * b
						+ Math.log(z / (C - z));
				...
				if (inner_iter > 0) // update w
				{
					...
					alpha[ind2] = C - z;
					for (Feature xi : prob.x[i]) {
						w[xi.getIndex() - 1] += sign * (z - alpha_old) * yi
								* xi.getValue();
					}
				}
			}
		...
		if (param.solverType == SolverType.L2R_L2LOSS_SVR || //
			...
			if (param.solverType == SolverType.MCSVM_CS) {
				...
				if (nr_class == 2) {
					...
					train_one(sub_prob, param, model.w, weighted_C[0],
							weighted_C[1]);
				} else {
	...
	private static void train_one(Problem prob, Parameter param, double[] w,
		...
		double primal_solver_tol = eps * Math.max(Math.min(pos, neg), 1)
				/ prob.l;
